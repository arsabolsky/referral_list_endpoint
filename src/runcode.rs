...
use serde_json;
use crate::env::Env;  // Assuming the Env struct is in `env.rs`


pub fn check_for_runcode() -> Option<Env> {
    let base64_string: String = Input::with_theme(&ColorfulTheme::default())
...
pub fn build_base64_runcode_from_env(env: &Env) -> Option<String> {
    ...
    match Select::with_theme(&ColorfulTheme::default())
        .with_prompt(
            "Do you want your configuration as a runcode?"
        )
        .default(1)
        .items(selections)
        .interact()
